 ...
 */
var docs_json_promise = undefined;
var docs_json_fuse = undefined;
var docs_cur_query = undefined;

function handleDocumentationSearch(searchQuery) {
    docs_cur_query = searchQuery;
    if (typeof docs_json_promise === 'undefined') {
        docs_json_promise = fetch('/en/docs/vcpkg-docs.json');
        docs_json_promise.then((res) => {
            return res.json();
        }).then((json) => {
            var options = {
                findAllMatches: true,
            ...
            docs_json_fuse = new Fuse(json, options);
            updateDocumentationSearch();
        });
    }
    else if (typeof docs_json_fuse !== 'undefined') {
        updateDocumentationSearch();
    }
}
function updateDocumentationSearch() {
    var searchResult = docs_json_fuse.search(docs_cur_query);
